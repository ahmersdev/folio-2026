...
export default function usePreloader() {
  ...
  useEffect(() => {
    ...
    setDisplayProgress((current) =>
      Math.max(current, Math.min(100, Math.round(progress))),
    );
  ...
  useEffect(() => {
    const bucket = Math.floor(displayProgress / ANNOUNCE_STEP) * ANNOUNCE_STEP;
    setAnnouncedProgress((current) => Math.max(current, bucket));
  }, [displayProgress]);
